...
{   
    ...
    {   
        ...
        $subject_categories = Subjectcategory::whereIn('is_active', [1, 2])->pluck('subjectcategory', 'id');

        $this->subjects = Subject::where('status', 1)
        ->when($this->patternclass_id, function ($query) { return $query->where('patternclass_id', $this->patternclass_id); })
        ->when($this->subjectcategory_id, function ($query) {  return $query->where('subjectcategory_id', $this->subjectcategory_id); })
