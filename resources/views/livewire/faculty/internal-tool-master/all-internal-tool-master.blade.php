<div>
    @if ($mode == 'add')
        <div>
            ...
            <x-form wire:submit="save()">
                @include('livewire.faculty.internal-tool-master.internaltool-form')
            </x-form>
        </div>
    @elseif($mode == 'edit')
    ...
    @elseif($mode == 'view')
        ...
        @include('livewire.faculty.internal-tool-master.view-form')
    @elseif($mode == 'all')
        <div>
                                    ...
                                    <x-table.td>
                                        @if (!$internal_tool->deleted_at)
                                            @if ($internal_tool->status === 1)
                                                <x-table.active wire:click="status({{ $internal_tool->id }})" />
                                            @else
                                                <x-table.inactive wire:click="status({{ $internal_tool->id }})" />
                                            @endif
                                        @endif
                                    </x-table.td>
                                    <x-table.td>
