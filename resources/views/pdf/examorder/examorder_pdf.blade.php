...
<body>
    ...
    <table border="1" cellpadding="10" cellspacing="0" width="680px">
            ...
            <tr>
                ...
                <td>{{$examorder->exampanel->subject->subject_code}}{{" ".$examorder->exampanel->subject->subject_name}}

                </td>
               <td>
                    <div>{{$examorder->exampanel->subject->exampanels->where('examorderpost_id','1')
                        ->where('active_status','1')->first()->faculty->faculty_name??''}}</div>
                    <div style="font-weight:regular;">{{$examorder->exampanel->subject->exampanels->where('examorderpost_id','1')
                        ->where('active_status','1')->first()->faculty->college->college_name??''}}</div>
                    <div> Mob:{{$examorder->exampanel->subject->exampanels->where('examorderpost_id','1')->where('active_status','1')->first()->faculty->mobile_no??''}}</div>
                    <div> Email:{{$examorder->exampanel->subject->exampanels->where('examorderpost_id','1')->where('active_status','1')->first()->faculty->email??''}}</div>
                </td>
                <td>
