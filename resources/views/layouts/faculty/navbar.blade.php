    ...
    <header class="relative bg-white dark:bg-darker">
        <div class="flex items-center justify-between p-2 border-b dark:border-primary-darker">
            ...
            <nav aria-label="Secondary" class=" z-50 hidden space-x-2 md:flex md:items-center">
                ...
                <!--Desktop Faculty avatar button -->
                <div x-cloak class="relative" x-data="{ open: false }">
                    <button @click="open = !open; $nextTick(() => { if(open){ $refs.facultyMenu.focus() } })" type="button" aria-haspopup="true" :aria-expanded="open ? 'true' : 'false'" class="transition-opacity duration-200 rounded-full dark:opacity-75 dark:hover:opacity-100 focus:outline-none focus:ring dark:focus:opacity-100">
                        <span class="sr-only">Faculty menu</span>
                        <img class="w-9 h-9 rounded-full" src="{{ asset('img/no-img.png') }}" alt="Faculty" />
                    </button>

                    <!-- Desktop Faculty dropdown menu -->
                    <div x-show="open" x-ref="facultyMenu" x-transition:enter="transition-all transform ease-out" x-transition:enter-start="translate-y-1/2 opacity-0" x-transition:enter-end="translate-y-0 opacity-100" x-transition:leave="transition-all transform ease-in" @keydown.escape="open = false" class="absolute right-0 w-48 py-1 bg-white rounded-md shadow-lg top-12 ring-1 ring-black ring-opacity-5 dark:bg-dark focus:outline-none" tabindex="-1" role="menu" aria-orientation="vertical" aria-label="Faculty menu">
                        <a wire:navigate href="{{ route('faculty') }}" role="menuitem" class="block px-4 py-2 text-sm text-gray-700 transition-colors hover:bg-gray-100 dark:text-light dark:hover:bg-primary">
                            Faculty Home
                         </a>
                        @auth('faculty')
                            {{-- <p class="block px-4 py-2 text-sm text-gray-700 transition-colors hover:bg-gray-100 dark:text-light dark:hover:bg-primary">{{ auth()->guard('faculty')->faculty()->faculty_name }}</p>
                            <hr> --}}
                            <a wire:navigate href="{{ route('faculty.dashboard') }}" role="menuitem" class="block px-4 py-2 text-sm text-gray-700 transition-colors hover:bg-gray-100 dark:text-light dark:hover:bg-primary">
                               Faculty Dashboard
                            </a>

                            <form method="POST" action="{{ route('faculty.logout') }}">
                                @csrf
                                <x-dropdown-link :href="route('faculty.logout')"  onclick="event.preventDefault(); this.closest('form').submit();" role="menuitem" class="block px-4 py-2 text-sm text-gray-700 transition-colors hover:bg-gray-100 dark:text-light dark:hover:bg-primary">
                                    Faculty Logout
                                </x-dropdown-link>
                            </form>
                        @else
                            <a wire:navigate href="{{ route('faculty.login') }}" role="menuitem" class="block px-4 py-2 text-sm text-gray-700 transition-colors hover:bg-gray-100 dark:text-light dark:hover:bg-primary">
                               Faculty Login
                            </a>
                        @endauth
                    </div>
            ...
            <nav x-transition:enter="transition duration-200 ease-in-out transform sm:duration-500"x-transition:enter-start="-translate-y-full opacity-0" x-transition:enter-end="translate-y-0 opacity-100" x-transition:leave="transition duration-300 ease-in-out transform sm:duration-500" x-transition:leave-start="translate-y-0 opacity-100" x-transition:leave-end="-translate-y-full opacity-0" x-show="isMobileSubMenuOpen" @click.away="isMobileSubMenuOpen = false" class=" z-50 absolute flex items-center p-4 bg-white rounded-md shadow-lg dark:bg-darker top-16 inset-x-4 md:hidden" aria-label="Secondary">
                ...
                <div x-cloak class="relative ml-auto" x-data="{ open: false }">
                    <button @click="open = !open" type="button" aria-haspopup="true" :aria-expanded="open ? 'true' : 'false'" class="block mx-auto transition-opacity duration-200 rounded-full dark:opacity-75 dark:hover:opacity-100 focus:outline-none focus:ring dark:focus:opacity-100">
                        <span class="sr-only">Faculty menu</span>
                        <img class=" items-center w-9 h-9 rounded-full" src="{{ asset('img/no-img.png') }}" alt="Faculty" />
                    </button>

                    <!-- Mobile Faculty dropdown menu -->
                    <div  x-show="open" x-transition:enter="transition-all transform ease-out" x-transition:enter-start="translate-y-1/2 opacity-0" x-transition:enter-end="translate-y-0 opacity-100" x-transition:leave="transition-all transform ease-in" x-transition:leave-start="translate-y-0 opacity-100" x-transition:leave-end="translate-y-1/2 opacity-0" @click.away="open = false" role="menu" aria-orientation="vertical" aria-label="Faculty menu">
                        ...
                        @auth('faculty')
                            ...
                            <form method="POST" action="{{ route('faculty.logout') }}">
                                @csrf
                                <x-dropdown-link :href="route('faculty.logout')"  onclick="event.preventDefault(); this.closest('form').submit();" role="menuitem" class="block px-4 py-2 text-sm text-start text-gray-700 transition-colors hover:bg-gray-100 dark:text-light dark:hover:bg-primary">
                                    <span class="ml-2 duration-300 ease-in-out">Faculty Logout</span>
                                </x-dropdown-link>
                            </form>
                        @else
                            <a wire:navigate href="{{ route('faculty.login') }}" role="menuitem" class="block px-4 py-2 text-sm text-gray-700 transition-colors hover:bg-gray-100 dark:text-light dark:hover:bg-primary">
                                <span class="ml-2 duration-300 ease-in-out">Faculty Login</span>
                            </a>
                        @endauth
                    </div>
                </div>
        ...
        <div class="border-b md:hidden dark:border-primary-darker" x-show="isMobileMainMenuOpen" @click.away="isMobileMainMenuOpen = false">
            <nav aria-label="Main" class="px-2 py-4 space-y-2">
                <a wire:navigate href="{{ route('faculty') }}" :class="{ 'bg-primary-100 dark:bg-primary': '{{ request()->routeis('faculty') }}' }" class="flex items-center p-2 px-5 text-gray-500 transition-colors rounded-md dark:text-light hover:bg-primary-100 dark:hover:bg-primary">
                    <span aria-hidden="true">
                        <span aria-hidden="true">
                    ...
                    <span class="ml-2 duration-300 ease-in-out">Faculty Home</span>
                </a>
                @auth('faculty')
                    <a wire:navigate href="{{ route('faculty.dashboard') }}" :class="{ 'bg-primary-100 dark:bg-primary': '{{ request()->routeis('faculty.dashboard') }}' }" class="flex items-center p-2 px-5 text-gray-500 transition-colors rounded-md dark:text-light hover:bg-primary-100 dark:hover:bg-primary">
                        <span aria-hidden="true">
                            <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 5a1 1 0 011-1h14a1 1 0 011 1v2a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM4 13a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H5a1 1 0 01-1-1v-6zM16 13a1 1 0 011-1h2a1 1 0 011 1v6a1 1 0 01-1 1h-2a1 1 0 01-1-1v-6z" />
                            </svg>
                        </span>
                        <span class="ml-2 duration-300 ease-in-out">Faculty Dashboard</span>
                    </a>
                    <form  method="POST" action="{{ route('faculty.logout') }}">
                        @csrf
                        <x-dropdown-link :href="route('faculty.logout')"  onclick="event.preventDefault(); this.closest('form').submit();" :class="{ 'bg-primary-100 dark:bg-primary': '{{ request()->routeis('faculty.logout') }}' }" class="flex items-center  p-2 px-5 text-gray-500 transition-colors rounded-md dark:text-light hover:bg-primary-100 dark:hover:bg-primary">
                            <span aria-hidden="true">
                                <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                @else
                    <a wire:navigate href="{{ route('faculty.login') }}" :class="{ 'bg-primary-100 dark:bg-primary': '{{ request()->routeis('faculty.login') }}' }" class="flex items-center p-2 px-5 text-gray-500 transition-colors rounded-md dark:text-light hover:bg-primary-100 dark:hover:bg-primary">
                        <span aria-hidden="true">
                            <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
