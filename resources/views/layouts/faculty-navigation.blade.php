        ...
        {{-- <x-dropdown align="right" width="48">
          ...
          <x-slot name="content">
            ...
            <form method="POST" action="{{ route("faculty.logout") }}">
              ...
              <x-dropdown-link :href="route("faculty.logout")" onclick="event.preventDefault();
        ...
        <x-dropdown align="right" width="48">
            <x-slot name="trigger">
                <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-white dark:text-gray-400 bg-gray-700 dark:bg-gray-800 hover:text-gray-300 dark:hover:text-gray-300 focus:outline-none transition ease-in-out duration-150">
                    {{-- <div>{{Auth::guard('student')->user()->student_name }}</div> --}} student
    ...
    <div class="border-t border-gray-200 pb-1 pt-4 dark:border-gray-600">
      ...
      <div class="mt-3 space-y-1">
        ...
        <form method="POST" action="{{ route("faculty.logout") }}">
          ...
          {{-- <x-responsive-nav-link :href="route("faculty.logout")" onclick="event.preventDefault(); this.closest('form').submit();">
            {{ __(" Faculty Log Out") }}
          </x-responsive-nav-link> --}}
        </form>
      </div>
