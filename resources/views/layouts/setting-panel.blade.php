<div>
  ...
  <!-- Panel -->
  <section x-transition:enter="transition duration-300 ease-in-out transform sm:duration-500" x-transition:enter-start="translate-x-full" x-transition:enter-end="translate-x-0"x-transition:leave="transition duration-300 ease-in-out transform sm:duration-500" x-transition:leave-start="translate-x-0" x-transition:leave-end="translate-x-full" x-ref="settingsPanel" tabindex="-1" x-show="isSettingsPanelOpen" @keydown.escape="isSettingsPanelOpen = false" class="fixed inset-y-0 right-0 z-20 w-full max-w-xs bg-white shadow-xl focus:outline-none dark:bg-darker dark:text-light sm:max-w-md" aria-labelledby="settinsPanelLabel">
    <div class="absolute left-0 -translate-x-full transform p-2">
      <!-- Close button -->
      <button @click="isSettingsPanelOpen = false" class="rounded-md bg-red-600 p-2 text-white focus:outline-none focus:ring">
    ...
    <div class="flex h-screen flex-col">
      <!-- Panel header -->
      <div class="flex flex-shrink-0 flex-col items-center justify-center space-y-4 border-b px-4 py-8 dark:border-primary-dark">
      ...
      <div class="flex-1 overflow-hidden hover:overflow-y-auto">
        ...
        <div class="space-y-4 p-4 md:p-8">
          ...
          <div class="flex items-center space-x-8">
            <!-- Light button -->
            <button @click="setLightTheme" class="flex items-center justify-center space-x-4 rounded-md border px-4 py-2 transition-colors hover:border-gray-900 hover:text-gray-900 focus:outline-none focus:ring focus:ring-primary-lighter focus:ring-offset-2 dark:border-primary dark:hover:border-primary-light dark:hover:text-primary-100 dark:focus:ring-primary-dark dark:focus:ring-offset-dark" :class="{ 'border-gray-900 text-gray-900 dark:border-primary-light dark:text-primary-100': !isDark, 'text-gray-500 dark:text-primary-light': isDark }">
              <span>
            ...
            <!-- Dark button -->
            <button @click="setDarkTheme" class="flex items-center justify-center space-x-4 rounded-md border px-4 py-2 transition-colors hover:border-gray-900 hover:text-gray-900 focus:outline-none focus:ring focus:ring-primary-lighter focus:ring-offset-2 dark:border-primary dark:hover:border-primary-light dark:hover:text-primary-100 dark:focus:ring-primary-dark dark:focus:ring-offset-dark" :class="{ 'border-gray-900 text-gray-900 dark:border-primary-light dark:text-primary-100': isDark, 'text-gray-500 dark:text-primary-light': !isDark }">
              <span>
                <svg class="h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="space-y-4 p-4 md:p-8">
          ...
          <div>
            ...
            <button @click="setColors('fuchsia')" class="h-10 w-10 rounded-full" style="background-color: var(--color-fuchsia)"></button>
            <button @click="setColors('blue')" class="h-10 w-10 rounded-full" style="background-color: var(--color-blue)"></button>
            <button @click="setColors('violet')" class="h-10 w-10 rounded-full" style="background-color: var(--color-violet)"></button>
          </div>
        </div>
      </div>
    </div>
