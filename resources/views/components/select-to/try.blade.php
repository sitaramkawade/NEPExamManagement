@props([ 'options' => null, 'value' => null ,'name'=>false])

<div  class=" inline-flex flex-col mt-4 " x-data="selecttodata() " x-init="setInitial({{ $value }})">
  <input type="hidden"   {{ $attributes->merge() }}  x-bind:value="selected.value" >
  <input class="w-1/2"  type="search" x-model="search" placeholder="Click to search..." @click="showOptions" @search="clearSearch" />
  <div class="relative" x-show="optionsVisible" @click.away="hideOptions">
    <div class="absolute z-50 border p-2 w-1/2 overflow-y-scroll bg-gray-200 max-h-60 grid">
      <template x-for="option in filteredOptions()">
        <a @click.prevent="selectOption(option)" x-html="highlight(option.label)" class="cursor-pointer hover:bg-blue-200 px-2">
        </a>
      </template>
    </div>
  </div>
